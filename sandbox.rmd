...
## Changing column names in dat file
```{r}
summary(rawsd)
summary(dat)
names(dat)[names(dat)=="FishStockName"] <- "StockCode"
dat[,c("StockCode")]
rawsd[,c("StockCode")]


newtab <- dplyr::left_join(rawsd, dat, by=NULL, copy=TRUE)

newtab[,c("StockCode","AssessmentKey", "Description", "DataCategory")]
